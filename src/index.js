...
const {
  Client, Collection, GatewayIntentBits, ApplicationCommandPermissionType,
} = require('discord.js');
...
class DiscordBot extends Client {
  ...
  loadComponents() {
    ...
    const buttons = fs.readdirSync(path.join(__dirname, 'components')).filter((fileName) => fileName.endsWith('.js'));
  ...
  loadModals() {
    ...
    const modals = fs.readdirSync(path.join(__dirname, 'modals')).filter((fileName) => fileName.endsWith('.js'));
