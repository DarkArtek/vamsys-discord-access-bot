...
const { AIRLINE } = require('../const/conf');

const ModalResponse = require('../struct/ModalResponse');
const VamsysUser = require('../struct/VamsysUser');
    ...
    try {
      ...
      if (!vamsysUser.exists || !vamsysUser.doesNameMatch(userResponse.name)) {
        ...
        return await interaction.followUp({
          content: 'Incorrect Details. Please try again, ensuring that you have entered your details exactly as requested above.',
          ephemeral: true,
        });
      }
      ...
      if (roleRemovalEnabled) {
        ...
        userRoles = userRoles.filter((role) => serverConfiguration.roleRemoval.roleId.indexOf(role) === -1);
      }
